		...
		return new Promise((resolve, reject) => {
			...
			mkdirp(destinationPath)
				.then(() => {
					...
					let req = request(distributionUrl)

						// TODO: Implement progress notification
						// .on('request', () => receivedSize = 0)
						// .on('response', (response) => totalSize = Number(response.headers['content-length']) || null)
						// .on('data', (data) => recievedSize += data.length))

						.on('end', () => logger.debug('Downloading adb finished'))
