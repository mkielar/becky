...
class Scanner {
	...
	_processStat(relativePath, total, stat) {
		...
		logger.info("Processing [%s]", sourceStat.sourcePath);
	...
	_missingOrModified(sourceStat) {
		...
		let relativePath = sourceStat.relativePath;
		let name = sourceStat.name;

		let destinationPath = Path.join(this._folderConfiguration.destination, sourceStat.relativePath, sourceStat.name);
		
		return Fs.statAsync(destinationPath)
		
			.then((destinationStat) => self._differ(sourceStat, destinationStat))
